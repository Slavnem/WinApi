...
// Gerekli şeyleri (buton, sabit metin, yazı metni, combo kutusu) oluşturuyoruz
...
// Programı çalıştırma fonksiyonu başlangıcı
int CalistirProgram(void)
{
	...
	// bu yazılanları stirng e çeviriyoruz ve birleştiriyoruz
...
{
	// başlama bilgisis ve işlem bilgisi oluşturuyoruz
	STARTUPINFO si;
	PROCESS_INFORMATION pi;

	// zeromemory ile bellekte yer açıyoruz
	ZeroMemory(&si, sizeof(si));
	si.cb = sizeof(si);
	ZeroMemory(&pi, sizeof(pi));

	// yazılan metini alıyoruz
...
void BulProgram(void)
{
	...
	// dosya açmak için bazı özellikleri belirtiyoruz
	...
	// dosyayı açıyoruz ve ekrandaki yazıya dosya konumunu yazdırıyoruz
...
{
	// renk değiştirip, cmd ekranında uygulama listesinin gösteriyoruz
	system("color 0a && tasklist && pause");
} // Çalışan uygulamalar gösterme fonksiyonu sonu

// Bilgisayar tamamen kapatma zamanlama fonksiyonu başlangıcı
...
// Bilgisayar oturum kapatma zamanlama fonksiyonu başlangıcı
...
void setZamanlamaYenidenBaslat(void)
{
	...
	system(ch_str_suresecim);
} // Bilgisayar yeniden başlatma zamanlama fonksiyonu sonu
...
// Bilgisayar kitleme zamanlama fonksiyonu başlangıcı
...
// Bilgisayar zamanlama iptal fonksiyonu başlangıcı
void setZamanlamaİptal(void)
{
	char sure[255];
	string str_t = "timeout /t ";
...
// İşlem öldürme fonksiyonu başlangıcı
...
} // İşlem öldürme fonksiyonu sonu


// Disk tamir fonksiyonu başlangıcı
void DiskTamir(void)
{
	ifstream disk_tamir;
	disk_tamir >> noskipws;
...
// Alternatif uygulama zamanlama fonksiyonu başlangıcı
